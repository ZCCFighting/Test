...
class StockDialog(QWidget):
    def __init__(self, parent=None):
        ...
        self.shapeComboBox = QComboBox()#下拉列表框
        ...
        self.widthSpinBox = QSpinBox()#计算器
        ...
        self.penStyleComboBox = QComboBox()#下拉列表框
        ...
        self.penCapComboBox = QComboBox()#下拉列表框
        ...
        self.penJoinComboBox = QComboBox()#下拉列表框
        ...
        self.brushStyleComboBox = QComboBox()#下拉列表框
        ...
        self.shapeComboBox.activated.connect(self.slotShape)    #设置形状

        self.widthSpinBox.valueChanged.connect(self.slotPenWidth)    #设置笔的线宽

        self.penColorPushButton.clicked.connect(self.slotPenColor)     #设置笔的颜色

        self.penStyleComboBox.activated.connect(self.slotPenStyle)    #设置笔的风格

        self.penCapComboBox.activated.connect(self.slotPenCap)    #设置笔的顶端

        self.penJoinComboBox.activated.connect(self.slotPenJoin)    #设置笔的连接点

        self.brushStyleComboBox.activated.connect(self.slotBrush)    #设置画刷的风格

        self.brushColorPushButton.clicked.connect(self.slotBrushColor)    #设置画刷的颜色

 

        self.slotShape(self.shapeComboBox.currentIndex())   #设置形状（初始化）

        self.slotPenWidth(self.widthSpinBox.value())    #设置笔的线宽（初始化）

        self.slotBrush(self.brushStyleComboBox.currentIndex())  #设置画刷的风格（初始化）
...
class PaintArea(QWidget):
    ...
    def __init__(self):
        ...
        self.setMinimumSize(400, 400)#背景大小

        self.pen = QPen()#定义笔

        self.brush = QBrush()#定义画刷
    ...
    def paintEvent(self, QPaintEvent):
        ...
        rect = QRect(50, 100, 300, 200)#代表一个矩形区域,实现在QtCore库,是一个QPoint对象和一个QSize对象的组合,QPoint对象是它的左上角的坐标,QSize对象则是它的大小
        ...
        if self.shape == "Line":
            ...
            p.drawPixmap(0, 50, QPixmap("images/qt-logo.jpg"))
